...
- **インポート**: 「↓ インポート」ボタンでファイル選択ダイアログを開き、JSONを読み込んで既存タスクに**マージ（追加）**する（上書きではない）
- **重複対策**: インポートするタスクのIDが既存と衝突する場合は `uid()` で新しいIDを発行してから追加
- インポート後は `saveState()` と `renderTasks()` を呼んで即反映
